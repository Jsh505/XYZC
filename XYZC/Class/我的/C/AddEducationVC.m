...
@interface AddEducationVC () <UITableViewDelegate, UITableViewDataSource>
{
    BOOL _isBegin;
}
...
{
    //确认
    if (self.schoolTF.text.length == 0 || self.typeTF.text.length == 0 || [self.xueliLB.text isEqualToString:@"请选择"] || [self.beginTimeLB.text isEqualToString:@"请选择"] || [self.infoTextView.text isEqualToString:@"请选择"])
    {
    ...
    [parametersDic setObject:self.schoolTF.text forKey:@"school"];
    [parametersDic setObject:self.typeTF.text forKey:@"major"];
    [parametersDic setObject:self.xueliLB.text forKey:@"education"];
    [parametersDic setObject:self.beginTimeLB.text forKey:@"enrolmentTime"];
    [parametersDic setObject:self.endTimeLB.text forKey:@"graduationTime"];
    [parametersDic setObject:self.infoTextView.text forKey:@"associationActivity"];
    if (self.model)
    {
        //修改
    ...
    [PPNetworkHelper POST:@"addOrUpdateEdu.app" parameters:parametersDic hudString:@"提交中..." success:^(id responseObject)
    {
...
- (void)rightBarButtonClicked
{
    ...
    [PPNetworkHelper POST:@"deleteEduByKey.app" parameters:parametersDic hudString:@"删除中..." success:^(id responseObject)
    {
        [self.navigationController popViewControllerAnimated:YES];
        //发送消息
        [[NSNotificationCenter defaultCenter]postNotification:[NSNotification notificationWithName:@"MyResumeRefshData" object:nil userInfo:nil]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)
    {
        NSArray * titleArray = @[@"学校",@"主修专业",@"学历",@"入学时间",@"毕业时间"];
        if (indexPath.row == 1 || indexPath.row == 0)
        {
            ...
            if (indexPath.row == 1)
            {
                if (self.model)
                {
                    cell.textField.text = [self.model objectForKey:@"major"];
                }
                self.typeTF = cell.textField;
            }
            else
            {
                if (self.model)
                {
                    cell.textField.text = [self.model objectForKey:@"school"];
                }
                self.schoolTF = cell.textField;
            }
        ...
        else
        {
            ...
            switch (indexPath.row)
            {
                case 2:
                {
                    if (self.model)
                    {
                        cell.infoLB.text = [self.model objectForKey:@"education"];
                    }
                    self.xueliLB = cell.infoLB;
                    break;
                }
                case 3:
                {
                    if (self.model)
                    {
                        cell.infoLB.text = [self.model objectForKey:@"enrolmentTime"];
                    }
                ...
                case 4:
                {
                    if (self.model)
                    {
                        cell.infoLB.text = [self.model objectForKey:@"graduationTime"];
                    }
    ...
    else
    {
        ...
        cell.titleLB.text = @"在校经历";
        if (self.model)
        {
            cell.infoTextView.text = [self.model objectForKey:@"associationActivity"];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        //学历
        if ([self.xueliLB.text isEqualToString:@"请选择"])
        {
            self.xueliLB.text = @"高中";
        }
        __weak typeof(self) weakSelf = self;
        [BRStringPickerView showStringPickerWithTitle:@"请选择学历" dataSource:@[@"高中",@"专科",@"本科",@"硕士",@"博士"] defaultSelValue:[self.xueliLB.text isEqualToString:@"高中"] ? @"高中" : self.xueliLB.text isAutoSelect:YES resultBlock:^(id selectValue)
         {
             weakSelf.xueliLB.text = selectValue;
         }];
    }
